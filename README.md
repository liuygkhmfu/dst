...
- 设置页可修改默认输出/导出目录、API 地址、API Key、模型、并发和模拟模式；描述词设计按“尺寸图 / 摆放图 / 场景图”三个分页分别维护专属全局约束和各张图片的完整内置描述词，设置保存在本地 `.env` 与 `data/runtime_config.json`。
- GPT-5.5 会按三种工作流分批分析：尺寸图只接收尺寸字段，摆放图只接收数量与摆放要求，普通场景图不继承摆放要求，只有图11接收自定义场景。最终提示词自动规范为中文。
...
备份时关闭应用后复制整个 `data/` 目录即可。数据库保存项目相对路径，因此整体移动项目目录后仍可恢复。
