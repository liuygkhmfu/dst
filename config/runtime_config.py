...
DEFAULT_FUNCTION_TEMPLATES: list[dict[str, str]] = [
    {
        ...
        "prompt": (
            ...
            "三根尺寸线必须分别使用英文标签LENGTH、WIDTH、HEIGHT。"
            "尺寸标注必须精准对应产品长边、宽边和高边的位置延伸，具有机械制图中轴测图尺寸线的空间对应感。"
            "尺寸文字要求：最终成图中所有可见文字必须全部使用英文，严禁出现中文、汉字或中英混排；"
            "仅允许出现PRODUCT SIZE、LENGTH、WIDTH、HEIGHT、cm、inch以及用户提供或自动换算的阿拉伯数字。"
            "尺寸数值来自{{产品尺寸}}。用户提供明确尺寸数值时，必须同时显示cm和inch，且清楚区分主辅单位；"
            "用户上传的原始数值和原始尺寸单位使用较大、明亮的产品同色字体，自动换算的另一单位尺寸使用较小黑色字体。"
            "如果{{产品尺寸}}为空或未提供明确数值，绝对不要编造厘米或英寸数值，仅展示三根尺寸线及英文标签LENGTH、WIDTH、HEIGHT。"
